...
namespace metrisca {
    ...
    {
        return sizeof(TraceDatasetHeader) + this->m_Traces.Size() + this->m_Plaintexts.Size() + this->m_Keys.Size() + this->m_Ciphertexts.Size();
    }
    ...
    int TraceDataset::LoadFromFile(const std::string& filename)
    {
        std::ifstream file;
        ...
        if(file)
        {
            ...
            file.read((char*)&this->m_Header, sizeof(TraceDatasetHeader));
            if(m_Header._MagicValue != DATASET_HEADER_MAGIC_VALUE)
            {
            ...
            switch(this->m_Header.PlaintextMode)
            {
                ...
                case PlaintextGenerationMode::RANDOM: {
                    plaintext_count = this->m_Header.NumberOfTraces;
                    plaintext_load_count = this->m_Header.NumberOfTraces;
                } break;
            ...
            this->m_Plaintexts = Matrix<uint8_t>(this->m_Header.PlaintextSize, plaintext_count);
            file.read((char*)this->m_Plaintexts.Data(), this->m_Header.PlaintextSize * plaintext_load_count * sizeof(uint8_t));
            ...
            this->m_Keys = Matrix<uint8_t>(this->m_Header.KeySize, key_count);
            file.read((char*)this->m_Keys.Data(), this->m_Keys.Size());
            
            // Read trace data
            this->m_Traces = Matrix<int32_t>(this->m_Header.NumberOfTraces, this->m_Header.NumberOfSamples);
            file.read((char*)this->m_Traces.Data(), this->m_Traces.Size());

            // Generate the chained plaintexts if the mode is set to CHAINED
            if(this->m_Header.PlaintextMode == PlaintextGenerationMode::CHAINED) {
                this->GenerateChainedPlaintexts();
            }
            ...
            this->m_Ciphertexts = Matrix<uint8_t>(this->m_Header.PlaintextSize, plaintext_count);
            this->GenerateCiphertexts();
        }
        else
        {
    ...
    int TraceDatasetBuilder::Build(TraceDataset& out)
    {
        ...
        out.m_Traces = Matrix<int32_t>(this->NumberOfTraces, this->NumberOfSamples);
        ...
        out.m_Plaintexts = Matrix<uint8_t>(this->PlaintextSize, plaintext_count);
        for(size_t p = 0; p < plaintext_copy_count; ++p)
        {
            ...
            out.m_Plaintexts.SetRow(p, plaintext);
        }
        ...
        out.m_Header.PlaintextSize = this->PlaintextSize;
        out.m_Header.PlaintextMode = this->PlaintextMode;
        out.m_Header.KeySize = this->KeySize;
        out.m_Header.KeyMode = this->KeyMode;

        // Generate the plaintexts if the mode is set to CHAINED
        if(this->PlaintextMode == PlaintextGenerationMode::CHAINED)
        {
            out.GenerateChainedPlaintexts();
        }

        // Generate the ciphertexts
        // NOTE: It would be better to provide the size of the ciphertexts as parameters
        //       even if the plaintext and ciphertext sizes are often the same
        out.m_Ciphertexts = Matrix<uint8_t>(this->PlaintextSize, plaintext_count);
        out.GenerateCiphertexts();
