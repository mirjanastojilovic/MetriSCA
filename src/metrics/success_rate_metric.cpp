...
#include "success_rate_metric.hpp"
...
namespace metrisca {
    ...
    static int parseSuccessRateArguments(SuccessRateArguments& out, const CLIView& view, const CLIModel& model, const InvocationContext& context)
    {
        SuccessRateArguments result = {};
        ...
        Parameter orderParam = context.GetNamedParameter({"order", "or"});
        ...
        if(!datasetParam)
        {
            view.PrintError("Missing dataset parameter. See 'help metric success_rate'.\n");
            return SCA_INVALID_COMMAND;
        }

        if(!modelParam) 
        {
            view.PrintError("Missing model parameter. See 'help metric success_rate'.\n");
            return SCA_INVALID_COMMAND;
        }

        if(!distinguisherParam)
        {
            view.PrintError("Missing distinguisher parameter. See 'help metric success_rate'.\n");
            return SCA_INVALID_COMMAND;
        }

        if(!outParam)
        {
            view.PrintError("Missing out parameter. See 'help metric success_rate'.\n");
            return SCA_INVALID_COMMAND;
        }
        ...
        result.Order = orderParam.GetOrDefault(1);
        ...
        if(result.Order <= 0 || result.Order > 256)
        {
            view.PrintError("Invalid order value. Order must be between 1 and 256.\n");
            return SCA_INVALID_ARGUMENT;
        }
    ...
    int metrics::SuccessRateMetric(const CLIView& view, const CLIModel& model, const InvocationContext& context)
    {
        SuccessRateArguments args;
        int argParseResult = parseSuccessRateArguments(args, view, model, context);
        ...
        writer << "trace_count";
        writer << "success_rate_key" + std::to_string(args.CorrectKey);
        writer << csv::EndRow;
        ...
        for(size_t step = 0; step < distinguisherOutput.size(); step++)
        {
            ...
            writer << (ranks[args.CorrectKey] <= args.Order ? 1 : 0);
