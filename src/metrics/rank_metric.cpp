...
namespace metrisca {
    ...
    static int parseRankArguments(RankArguments& out, const CLIView& view, const CLIModel& model, const InvocationContext& context)
    {
        RankArguments result = {};
        ...
        if(!datasetParam)
        {
            view.PrintError("Missing dataset parameter. See 'help metric rank'.\n");
            return SCA_INVALID_COMMAND;
        }

        if(!modelParam) 
        {
            view.PrintError("Missing model parameter. See 'help metric rank'.\n");
            return SCA_INVALID_COMMAND;
        }

        if(!distinguisherParam)
        {
            view.PrintError("Missing distinguisher parameter. See 'help metric rank'.\n");
            return SCA_INVALID_COMMAND;
        }

        if(!outParam)
        {
            view.PrintError("Missing out parameter. See 'help metric rank'.\n");
            return SCA_INVALID_COMMAND;
        }
    ...
    int metrics::RankMetric(const CLIView& view, const CLIModel& model, const InvocationContext& context)
    {
        RankArguments args;
        int argParseResult = parseRankArguments(args, view, model, context);
        ...
        {
            writer << "rank_key" + std::to_string(i);
        }
        writer << csv::EndRow;
        ...
        for(size_t step = 0; step < distinguisherOutput.size(); step++)
        {
            ...
            for(uint32_t r = 0; r < ranks.size(); r++)
            {
                writer << ranks[r];
            }
